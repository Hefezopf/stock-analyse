...
do
    ...
    rm -rf "$indexSymbolFile"
    {
        ...
        if [ ! "$marketCapFromFile" = '?' ]; then
            ...
            echo "<div id='progress' style='background: rgba(240,236,236,255); border-radius: 13px;height: 24px; width: 98%; padding: 3px; text-align: left'>&nbsp;Markt Kapitalisierung&nbsp;$marketCapFromFile Mrd.€</div><br>"           
        fi
    ...
    if [ "$markerOwnStock" = '*' ] && [ "$buyingRate" ] ; then
        ...
        {   
            ...
            # Image Chart
            echo "<br><img width=\"55%\" id=\"intervalSectionImage$symbol\" style='display: none'></img><br>
...
fi
{
    ...
    echo "<img width=\"55%\" id=\"intervalSectionImageDAX\" style='display: none'></img><br>
