...
do
    ...
    do
        ...
        if [ "$valueRSI" -lt "$RSIBuyLevelParam" ] && [ "$isMACDNegativ" = '-' ]; then
            ...
            Out "Buy\tPosition:$RSIindex\t""$piecesPerTrade""pc\tRSI:$valueRSI\tQuote:$quoteAt€\tAmount=$amount€\tpiecesHold=$piecesHold\tWallet=$wallet€\tSymbol:$symbol" $OUT_SIMULATE_FILE "-e"
        ...
        if [ "$piecesHold" -gt 0 ] && [ "$stochAt" -gt "$StochSellLevelParam" ]; then
            ...
            quoteAt=$(printf "%.2f" "$quoteAt")
            Out "Sell\tPosition:$RSIindex\t""$piecesHold""pc\tStoch:$stochAt\tQuote:$quoteAt€\tSellAmount=$amount€\tSymbol:$symbol" $OUT_SIMULATE_FILE "-e"
    ...
    if [ "$piecesHold" -gt 0 ]; then
        ...
        amount=$(echo "$quoteAt $piecesHold" | awk '{print ($1 * $2)}')
        quoteAt=$(printf "%.2f" "$quoteAt")
        Out "Sell\t""$piecesHold""pc\tQuote:$quoteAt€\tSellAmount=$amount€\tSymbol:$symbol" $OUT_SIMULATE_FILE "-e" 
        sellAmountOverAll=$(echo "$sellAmountOverAll $amount" | awk '{print ($1 + $2)}')
        intermediateProzWin=$(echo "$amount $wallet" | awk '{print (($1 / $2 * 100)-100)}')
        intermediateProzWin=$(printf "%.1f" "$intermediateProzWin")
        wallet=$(echo "$amount $wallet" | awk '{print ($1 - $2)}') 
