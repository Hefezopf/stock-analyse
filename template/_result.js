// Global Varables
var token1 = 'ghp_';
var token2 = 'fpiBX3Xkd4P32MrChQnrzovTWSk5TQSzBKD6';
...
function calulateRealtimeDailyDiff(ele) {
    ...
    realtimeDailyDiff = realtimeDailyDiff + (parseFloat(difference) * parseInt(pieces));
}
...
function XX_FETCH_onContentLoaded(symbol, notationId, asset_type) {
    // Show local link, if on PC
    if (location.href.startsWith('file')) {
    ...
    .then(data => {
        ...
        var realTimeProzSymbol = data.contents.slice(positionProz1 - 6, positionProz1);
        ...
        // Gain
        let positionGain1 = data.contents.indexOf('&#160;%');
        var realTimeGainSymbol = data.contents.slice(positionGain1, positionGain1 + 200);
        ...
        var timeSymbol = data.contents.slice(positionTime1 + 4, positionTime1 + 12);
        var hoursSymbol = timeSymbol.slice(0, 2);
        var minutesSymbol = timeSymbol.slice(3, 5);
        var secondsSymbol = timeSymbol.slice(6, 8);
        var dateSymbol = data.contents.slice(positionTime1 - 9, positionTime1 - 1);
    ...
    .catch(error => {
        console.error('Error/Timeout retrieving current quote for: intervalSectionRealTimeQuote' + symbol + ' !!!' + error);
        counterFetchLoaded++; // For Spinner
    });
}
