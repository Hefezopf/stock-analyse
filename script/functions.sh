...
# Write the alarm x-axis file into alarm dir
# Input: ${x}
# Output: alarm/$symbol.txt file
WriteAlarmAbbrevXAxisFile() {
    ...
    if [ -f "$alarmSymbolDateFile" ]; then
        cp -f "$alarmSymbolDateFile" "$alarmSymbolFile"
    else
        if [ -f "$alarmSymbolDateBeforeFile" ]; then
            ...
            cp -f "$alarmSymbolFile" "$alarmSymbolDateFile"
        fi
    fi
}
