...
CurlSymbolName() {
    ...
    if [ ! "${#symbolName}" -gt 1 ]; then
        ...
        if ! [ "$symbolName" = 'null' ]; then
           echo "$_symbolParam""$(printf '\t')""$symbolName""$(printf '\t')""999999""$(printf '\t')""?""$(printf '\t')""\"--\"""$(printf '\t')""--""$(printf '\t')""--""$(printf '\t')""XETRA" | tee -a "$_tickerNameIdFileParam"
           # echo "$_symbolParam""$(printf '\t')""$symbolName""$(printf '\t')""999999""$(printf '\t')""?""$(printf '\t')""\"--\"""$(printf '\t')""--""$(printf '\t')""--""$(printf '\t')""XFRA" | tee -a "$_tickerNameIdFileParam"
...
WriteComdirectUrlAndStoreFileList() {
    ...
    if [ ! "$_id_notation" = "$ID_NOTATION_STORE_FOR_NEXT_TIME" ]; then
        ...
        {
            echo "<div id=\"firstlineId$symbol-111\" style=\"margin-top: 6px\">"
