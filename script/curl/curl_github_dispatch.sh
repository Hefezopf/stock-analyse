...
# Example: sh ./script/curl/curl_github_dispatch.sh "CEC" 1 offline 9 30
# Example: sh ./script/curl/curl_github_dispatch.sh "*GIS" 1 offline 9 30
# !!Only one symbol can be passed as parameter!! For example: This list is NOT possible: "INL BMW" 

if { [ -z "$1" ] || [ -z "$2" ] || [ -z "$3" ] || [ -z "$4" ]; } then
  echo "Not all parameters specified!"
  echo "Example: curl_github_dispatch.sh \"*GIS\" 1 offline 9 30"
  exit 1
fi
set -x
